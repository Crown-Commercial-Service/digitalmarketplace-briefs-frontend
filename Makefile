...
docker-build:
	...
	docker build -t digitalmarketplace/briefs-frontend --build-arg release_name=${RELEASE_NAME} .
	docker tag digitalmarketplace/briefs-frontend digitalmarketplace/briefs-frontend:${RELEASE_NAME}

.PHONY: docker-push
docker-push:
	$(if ${RELEASE_NAME},,$(eval export RELEASE_NAME=$(shell git describe)))
	docker push digitalmarketplace/briefs-frontend:${RELEASE_NAME}
